...
def compact_number(value: int) -> str:
    ...
    Thousands below 10 use one decimal (``1,6 k``), values below 100 use one
    decimal (``12,3 k``), and three-digit thousands use no decimal
    (``999 k``).  This keeps the result to at most three significant digits.
    Values from 999,500 round up to the millions band instead of displaying
    the misleading ``1 000 k``.
    """
    number = int(value)
...
def daily_series(
    ...
    return [
        (
            ...
            totals.get(start + timedelta(days=index), 0),
        )
