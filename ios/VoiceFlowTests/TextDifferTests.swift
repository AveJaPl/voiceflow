...
/// Testy hardware-free reconciliacji partiali — metodologia 1:1 z
/// `apps/mac/VoiceFlowTests/TextDifferTests.swift` (ten sam algorytm, ten
/// sam plik `Shared/TextDiffer.swift`, kompilowany bezpośrednio w tym
/// targecie — bez `@testable import`, patrz project.yml). Kryterium
/// weryfikacji #3 z docs/plans/ios-voiceflow-app.md.
final class TextDifferTests: XCTestCase {
    ...
    // MARK: - Testy specyficzne dla iOS: symulacja pełnej pętli przez ProxyTextInjector-owy model

    /// `ProxyTextInjector` operuje na `UITextDocumentProxy` (deleteBackward +
    /// insertText) — tu odtwarzamy tę samą arytmetykę na zwykłym Stringu, żeby
    /// sprawdzić, że plan z `diff()` da się bezpiecznie zastosować sekwencyjnie
    /// (kolejność: najpierw wszystkie usunięcia, potem wstawienie) tak samo,
    /// jak zrobi to `ProxyTextInjector.apply(_:)`.
    func testPlanAppliesCorrectlyWithDeleteThenInsertOrder() {
        let d = TextDiffer()
        _ = d.diff(toHypothesis: "mama myje")
        let plan = d.diff(toHypothesis: "mama myje ramę")

        var buffer = Array("mama myje")
        for _ in 0..<plan.deleteCount { buffer.removeLast() }
        buffer.append(contentsOf: plan.insert)
        XCTAssertEqual(String(buffer), "mama myje ramę")
    }
}
